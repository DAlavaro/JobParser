...
<aside>
   Крсовая работа по парсингу сайтов вакансий
</aside>


## В текущем проекте парсинг выполняется по сайтам
Проект позволяет получить по 100 самых близких к запросу вакансий с сайтов hh.ru и superjob.ru
Полученные вакансии объединяются в один список и сохраняются в файле vacancies.json

## Реализация проекта
- В проекте реализован абстрактный метод abstract_api.py для работы с API сайтов с вакансиями
- Реализовать классы, наследующиеся от абстрактного класса abstract_api.py классы headhunter.py и super_job.py для работы с API 
- Классы headhunter.py и super_job.py получают данные с API по ключевому слову и переводят полученные данные к стандартизированному списку
- Реализован абстрактный класс AbstractJsonVacancy для работы с вакансиями
- Класс JsonVacancy 
  - позволяет сохранять полученные вакансии в файл vacancies.json
  - получать количество найденных вакансий под заданные условия
  - фильтровать и сортировать вакансии
  - выводить в консоль вакансии под заданные условия
